...
#April 8, 2018 Codecademy
from random import randint

board = []

# Multiplies and appends ["O"] to empty list
for x in range(5):
  board.append(["O"] * 5)

  
# Removes brackets and commas from the list within the list
def print_board(board):
  for row in board:
    print " ".join(row)
print_board(board)
...
# Produce random coordinates for the ship
def random_row(board):
  return randint(0, len(board) - 1)
def random_col(board):
  return randint(0, len(board[0]) - 1)
...
# Assigns random coordinates with ship & prints them
ship_row = random_row(board)
ship_col = random_col(board)
print ship_row
print ship_col
...
# Directs traffic and controls player turns
for turn in range(4):
  print "Turn", turn + 1
  guess_row = int(raw_input("Guess Row: "))
  guess_col = int(raw_input("Guess Col: "))
